...
class SemanticMatcher:
    ...
    def match_job_description(self, resume_text, job_description):
        ...
        resume_str = str(resume_text) if not isinstance(resume_text, str) else resume_text
        job_str = str(job_description) if not isinstance(job_description, str) else job_description

        emb_resume = self.model.encode(resume_str, convert_to_tensor=True)
        emb_job = self.model.encode(job_str, convert_to_tensor=True)
    ...
    def encode_document(self, text):
        ...
        """
        # --- FIX STARTS HERE ---
        # 1. Check if input is a Dictionary (the source of your error)
        if isinstance(text, dict):
            # Extract the actual text string from the dictionary
            # It tries 'raw_text', then 'text', then falls back to stringifying the dict
            text = text.get('raw_text') or text.get('text') or str(text)

        # 2. Now safely check if it is a valid string
    ...
    def score_document_quality(self, document_text):
        ...
        """
        # Handle dict input here too just in case
        if isinstance(document_text, dict):
            document_text = document_text.get('raw_text', '')
    ...
    def calculate_profile_coherence(self, doc_embeddings):
        ...
        """
        # Filter out invalid embeddings (zeros)
        valid_embeddings = [e for e in doc_embeddings if np.any(e)]
        
        if not valid_embeddings or len(valid_embeddings) < 2:
            return 100  # Single document is perfectly coherent
        
        # Calculate pairwise cosine similarities
        embeddings_tensor = torch.tensor(np.vstack(valid_embeddings))
